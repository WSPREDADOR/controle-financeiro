...
async function saveDevice(deviceId) {
  ...
  if (error) {
    showAdminNotice(error.message, 'Nao foi possivel salvar');
    return;
  }
...
async function deleteDevice(device) {
  ...
  if (error) {
    ...
    showAdminNotice(error.message, 'Nao foi possivel excluir');
    return;
  }
...
async function sendCommand(deviceId, commandType, payload) {
  ...
  if (error) {
    showAdminNotice(error.message, 'Nao foi possivel enviar o comando');
    return;
  }
...
function showAdminNotice(message, title = 'Aviso') {
  const overlay = document.createElement('div');
  overlay.className = 'admin-dialog-overlay';
  overlay.innerHTML = `
    <section class="admin-dialog admin-dialog-notice" role="dialog" aria-modal="true" aria-labelledby="adminNoticeTitle">
      <button type="button" class="admin-dialog-close" aria-label="Fechar aviso" data-dialog-close>&times;</button>
      <span class="kicker">Painel admin</span>
      <h2 id="adminNoticeTitle">${escapeHtml(title)}</h2>
      <p class="admin-dialog-lead">${escapeHtml(message || 'Nao foi possivel concluir a acao agora.')}</p>
      <div class="admin-dialog-actions">
        <button type="button" data-dialog-close>Entendi</button>
      </div>
    </section>
  `;
